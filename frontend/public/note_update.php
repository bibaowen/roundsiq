...
if ($text==='') back($case_id, 'Note cannot be empty.', 'error');
...
if (!$owned) back($case_id, 'You do not have permission to edit this note.', 'error');

// Update
$upd = $pdo->prepare("UPDATE case_notes SET note=:n WHERE id=:id");
$upd->execute([':n'=>$text, ':id'=>$note_id]);

back($case_id, 'Note updated.');
